<template>
  <div class="ui-data-table flex flex-1 flex-column">
    ...
    <div class="data-table" ref="wrapper" :style="{margin:full?'0 -1rem':'0'}">
      <slot></slot>
...
export default {
  ...
  methods: {
    ...
    customQuery (resource, method, current, ...rest) {
      ...
      this.pending = true
      const condition = merge(this.showPagination ? { "page": { "current" : current, "limit": this.globalConfig.pageLimit }} : {}, ...rest)
